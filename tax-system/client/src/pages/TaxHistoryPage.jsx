...
export default function TaxHistoryPage() {
  ...
  // Years available
  const years = [...new Set(declarations.map((d) => d.taxYear))].sort((a, b) => b - a);

  // Filter
  const filtered = declarations.filter((d) => {
    const matchStatus = filterStatus === "all" || d.status === filterStatus;
    const matchYear = filterYear === "all" || String(d.taxYear) === filterYear;
                ...
                <div className="decl-main" onClick={() => setExpandedId(isExpanded ? null : decl._id)}>
                  <div className="decl-left">
                    <div className="decl-id">
                      <span className="decl-year">Năm {decl.taxYear}</span>
